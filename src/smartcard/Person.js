...
class Person extends EventEmitter {
  ...
  async getCid() {
    if (this._allData.cid == null) {
      this.setCid(
        (
          await ReaderUtils.runAndReadAllByteToString(this._card, APDU.GET.CID)
        ).substring(0, 13)
      );
    }
  ...
  async getFirst4CodeUnderPicture() {
    if (this._allData.first4CodeUnderPicture == null) {
      this.setFirst4CodeUnderPicture(
        await ReaderUtils.runAndReadAllByteToString(
          this._card,
          APDU.GET.FIRST_4_CODE_UNDERPICTURE
        )
      );
    }
  ...
  async getLast8CodeUnderPicture() {
    if (this._allData.last8CodeUnderPicture == null) {
      this.setLast8CodeUnderPicture(
        await ReaderUtils.runAndReadAllByteToString(
          this._card,
          APDU.GET.LAST_8_CODE_UNDERPICTURE
        )
      );
    }
  ...
  async getNameEN() {
    if (
      ...
      this.setNameEN(
        ReaderUtils.sharpToSpace(
          await ReaderUtils.runAndReadAllByteToString(
            this._card,
            APDU.GET.EN_FULLNAME
          )
        ).trim()
      );
    }
  ...
  async getGender() {
    if (this._allData.gender == null) {
      this.setGender(
        await ReaderUtils.runAndReadAllByteToString(this._card, APDU.GET.GENDER)
      );
    }
  ...
  async getDoB() {
    if (
      ...
      this.setDoB(
        await ReaderUtils.runAndReadAllByteToString(this._card, APDU.GET.DOB)
      );
    }
